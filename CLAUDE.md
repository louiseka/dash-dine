...
- **`src/data.ts`** (`menuArray`) is a leftover/unused duplicate of the menu data — the live menu used by the app is defined inline in `src/index.ts`. Don't assume `data.ts` is wired up before editing menu items.
